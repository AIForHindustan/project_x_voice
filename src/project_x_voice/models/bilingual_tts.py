...
class BilingualTTS(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.embedding = nn.Embedding(config["model"]["vocab_size"], config["model"]["hidden_dim"])
        self.encoder = nn.LSTM(config["model"]["hidden_dim"], config["model"]["hidden_dim"]//2, 
                             bidirectional=True, batch_first=True)
        
        self.attention = LocationAwareAttention(config["model"]["hidden_dim"], 128)
        self.duration_predictor = DurationPredictor(config["model"]["hidden_dim"])
        
        self.decoder = nn.LSTMCell(config["model"]["hidden_dim"] + config["model"]["n_mels"], 
    ...
    def forward(self, phonemes, mels):
        ...
        hx = torch.zeros(phonemes.size(0), self.decoder.hidden_size, device=phonemes.device)
        cx = torch.zeros_like(hx)
        prev_mel = torch.zeros(phonemes.size(0), self.config["model"]["n_mels"], device=phonemes.device)
        ...
        return torch.stack(mel_outputs, 1), torch.stack(attn_weights, 1), durations